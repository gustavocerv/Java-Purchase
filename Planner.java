...
* Lessons Learned: We learn how to use the tool, and can help us to identify the error and fix it
*   
...
public class Planner {
	...
	public static void createModule(Scanner sIn) {
		...
		while(!done) {
			...
			switch (choice) {
				case "1":			
					addPlan(sIn, "Packing");
					break;// Test 59 : the break was in comments //
				case "2":			
	...
	public static void addPlan(Scanner sIn, String planType) {
		...
		if (planType.equalsIgnoreCase("Packing")) {
			...
			PackingPlan packingPlan = new PackingPlan(planTitle, destination, travelers, tripType);// Test 88 : autopopulated was in disorder, with triptype and destination
	...
	public static void changeItemQty(Scanner sIn, ToDo plan) {
		...
		if ( changeQty < 0 && Math.abs(changeQty) >= plan.getAmount(item)) {plan.delete(item);} //103
		else {plan.change(item, changeQty);} // Test 152 : line 335 and 336, have be add in order to remove numbers in negative
	...
	public static void deleteItem(Scanner sIn, ToDo plan) {
		//Get the list of items (keys) from the plan HashMap and convert the Set<String> to an array String[].
		Set<String> itemList = plan.getItems();		//Get the Set<String> of keys from the HashMap.
		int setSize = itemList.size();// Test 103 : remove the -1 in line 354, setSize = itemList.size();			                                                //Find the number of items in the HashMap.
		String[] choices = new String[setSize];		//Create a new array String[] that is the same size as the HashMap.
	...
	public static int getValidInt(Scanner sIn, String question, String warning){
		...
		int intNum = 1;					//The integer var to hold the converted input
